...
Installed CLI pairs carry one value in the skill: the **CLI Contract version** of the namespace it was written against. The pin makes independent plugin and skill updates visible — an agent compares it with the `contractVersion` in the first answer and, on a difference, reads the live guide again and follows the guide over the skill. A change that bumps a namespace's version moves the pin of every skill written against it, in the same change.

Git-managed internal tooling uses the repository revision as its compatibility marker. Keep its CLI, skill, and human documentation current in the same change, and omit the namespace version and skill pin.
