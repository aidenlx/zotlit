...
- **`parseNote` signature grows** to `parseNote({ db, template, Turndown }, html, libraryID)`, where `db: NodeDatabaseClient | null` is resolved at the call site (`db.state === "ready" ? db.client : null`) so a degraded DB is a normal embedded-fallback path, not a throw. Declare the `TurndownService` global in `src/typings/obsidian-ex.d.ts`.
