...
| 8   | **(attachment import + UI done)** **Annot view + attachment import** _(alpha blocker)_ | `getAttachmentByKey`                                                                                       | Attachment import **shipped** (`AttachmentImportService` coordinator + `lib/` helpers + settings + note create/update/overwrite wiring; reflink+copy, size+mtime skip, `file://` URI fallback when disabled). React 19.2 + jotai `ItemView` UI **done** (per-leaf `createStore()` + `<Provider>`, `AnnotActions` context, mock data seed); live DB wiring + real `AnnotActions` impl + drag-insert still pending. (PDF outline deferred — see §3.2.) |
| →   | **v2 alpha ships**                                                     |                                                                                                            |                                                                                                                                                                                                                                                                                                                                         |
...
- `apps/obsidian/src/services/note-feature/`. Note **update** is a managed-region overwrite, **not** a port of v1's `update-note.ts` incremental merge (no CodeMirror `EditorState`, no block-ID diffing, no `isAnnotBlock`). The renamed `content` template (was `annots`) renders the annotation region; the `eta.ts` include rewrite auto-wraps it in `%%zt-managed%%` markers (code-injected, never in `.eta.md`). `update-note` re-renders that region and splices it via `app.vault.process` + merges frontmatter via `app.fileManager.processFrontMatter` (`@std/collections` `deepMerge`/`distinct`/`pick`, key-level merge preserving unmanaged keys); `overwrite-note` replaces the whole body behind a `ConfirmationModal` and is the missing-marker repair path. Both palette-only + `checkCallback`-gated on `zotero-key`. `zt-attachments` scoping + v1 numeric-ID migration are **deferred to post-alpha** (land with the attachment whitelist/blacklist UI that writes the key); alpha always uses all attachments. Full design: `HANDOFF.md` Phase D.
...
**Stage 8 — Annot view + attachment import** _(attachment import done; annot view pending)_
...
- `packages/db/src/lib/zt-path.ts`: `resolveAnnotCachePath({ annotKey, groupID, dataDir })` (options object) for annotation cache source paths; `attachmentAbsPath(attachment, ctx)` + `AttachmentPathContext` moved here from `note-feature/file-link.ts` — resolves any Zotero attachment (storage, linked-absolute, linked-base) to an absolute on-disk path. Exported via the dedicated `@zotlit/db/path` entry (depends on `node:path`, so it stays out of `index.ts`).
...
- `apps/obsidian/src/views/annot-view/` — React 19.2 view mounted via `createRoot`. Module layout:
  - `view.tsx` — `AnnotationView extends ItemView`: per-leaf `createStore()` on `#store`, `<Provider store={this.#store}>` wraps the tree; `contentEl.addClass("zt-root")` scopes Tailwind preflight; `onClose` calls `#root.unmount()`. Currently seeded with mock data from `mock.ts`; remaining work replaces the seed with live DB queries.
  - `store.ts` — view-model atoms (`docAtom`, `allAttachmentsAtom`, `attachmentIDAtom`, `annotationsAtom`, `tagsAtom`, `followAtom`) + derived `activeAttachmentAtom`. Narrow `AnnotItem` / `AtchItem` / `DocItem` / `TagItem` interfaces — a deliberate subset of `@zotlit/db` row shapes that the mock can satisfy without Temporal/position fields.
  - `AnnotView.tsx` — `AnnotView` root (toolbar + list), `Toolbar` (details / collapse / refresh / follow buttons), `FollowButton` (link icon + `"ob"`/`"zt"` mode label), `AttachmentSelector` (native `<select>` hidden when ≤1 attachment), `AnnotList` (CSS `@container` masonry via `columns-*` breakpoints).
  - `Annotation.tsx` — `Annotation` card (icon draggable header, colored `<blockquote>` excerpt, comment, tags); `Excerpt` switches on `annotationTypeToName(type)` (highlight/underline/text → `<p>`, image → `<img>`); `PageLabel` renders page label as a `zotero://open/` anchor when a backlink is available.
  - `actions.tsx` — `AnnotActions` interface (`onShowDetails`, `onMoreOptions`, `onDragStart`, `onRefresh`, `onSetFollow`, `getImgSrc`, `getBacklink`) + `AnnotActionsContext`. `mockAnnotActions` console-logs each action and returns a placeholder SVG for images; `getBacklink` already constructs the real `zotero://open/library/items/{parentKey}?page=…&annotation=…` URI. Remaining work replaces `mockAnnotActions` with real service wiring.
  - `register.ts` — `registerAnnotView(plugin)`: `registerView`, `addCommand` (`"open-annot-view"`), `addRibbonIcon` (`"highlighter"`). `activateView` opens/reveals in the right sidebar leaf.
...
_Remaining Stage 8 work:_

- Replace mock seed in `view.tsx` with live DB queries (`annotations`/`attachments`) reactive to active file + Zotero reader focus.
- Implement real `AnnotActions`: template render + drag-insert, `getImgSrc` via attachment import, Zotero reader follow mode.
