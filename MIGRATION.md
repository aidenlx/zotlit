...
- **Dead keys, no UI.** `template.update-overwrite` and `template.update-annot-block` have no v2 consumer (block-ID/overwrite-gating infra was removed in Stage 5), so they get no settings row — left in the schema/migration for round-trip, flagged as dead. `attachment.folder-path` empty is treated as the default folder (`ensureAttachmentFolder` relaxed to `!folderPath`).
