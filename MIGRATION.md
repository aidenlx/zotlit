...
| 8   | **Annot view + attachment import** _(alpha blocker)_                   | `getAttachmentByKey`                                                                                       | Attachment import **shipped** (`AttachmentImportService` coordinator + `lib/` helpers + settings + note create/update/overwrite wiring; reflink+copy, size+mtime skip, `file://` URI fallback when disabled). React 19.2 + jotai annot view + drag-insert still pending. (PDF outline deferred — see §3.2.)                               |
| →   | **v2 alpha ships**                                                     |                                                                                                            |                                                                                                                                                                                                                                                                                                                                         |
...
_Annot view (pending):_

- `apps/obsidian/src/services/annot-view/`: React 19.2 view mounted via `createRoot`; jotai atoms declared at module scope with a per-leaf `createStore()` on the `ItemView` subclass and `<Provider store={this.#store}>` wrapping the tree; drag-insert handler (reuses the shipped `AttachmentImportService`); reactive sync to active file + Zotero reader focus.
