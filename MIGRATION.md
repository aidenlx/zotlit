...
| 4   | (done) **NoteParser**                                                  | —                                                                                                          | Turndown converter + Zotero HTML rules; payload-only annotation marks; schema gate. Mark/color parsing lives in `@zotlit/db` (`zt-note-mark`, `zt-color`). Citation DB leg + `cite` render deferred to Stage 5; image embeds to Stage 9.                                                                                                  |
...
Converts Zotero 9 note HTML → Obsidian Markdown, resolving annotation excerpts to flat inline marks. Citation resolution is **deferred to Stage 5** (it exists only to feed `template.render("cite", …)`); image embeds to **Stage 9**. No DB queries this stage — annotation marks are payload-only.
...
_Note citation resolution (settled in Stage 4, deferred here)._ The `citation` rule ships as a pass-through in Stage 4; resolving it belongs here because the whole citekey chain exists only to feed `template.render("cite", …)`. The parsers are already shipped (`parseCitation` → `@zotlit/db` `parseCitationData` / `parseItemUri`); only the orchestrator wiring is new.
