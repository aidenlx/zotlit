...
`<case>` accepts a case id, a path under `cases/`, or an absolute path.

## Case layouts

The restored Workbench Case lives under `cases/zotlit-template-workspace/`. Read its
`README.md` for the historical source, evidence limits, and the current evaluation owner.
