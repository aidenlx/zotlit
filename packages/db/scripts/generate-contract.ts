...
await writeJson("ir.runtime.json", toRuntimeIR(ir));
for (const root of EMITTED_ROOTS) {
...
/** The IR a serializer ships: doc text pruned, emitter-only tables dropped. */
function toRuntimeIR(full: ContractIR): RuntimeContractIR {
  const { itemTypes: _itemTypes, ...runtime } = full;
  return pruneDocFields(runtime) as RuntimeContractIR;
}

function pruneDocFields(value: unknown): unknown {
  if (Array.isArray(value)) return value.map(pruneDocFields);
  if (typeof value !== "object" || value === null) return value;
  return Object.fromEntries(
    Object.entries(value)
      .filter(([key]) => key !== "description" && key !== "examples")
      .map(([key, entry]) => [key, pruneDocFields(entry)]),
  );
}
