...
**Action**:
A verb in the `zotlit/*` namespace (`open`, `open-attachment`, `update`, `update-many`, `update-all`, `import-note`, `import-notes`, `import-all-notes`, `explore`). An action id plus its required query fields is a frozen contract on the URL transport; a breaking change is expressed as a **new** action id, never by mutating an existing one.
