...
- In JSDoc comment, prefer using `@see` (one tag per reference) to point at external files, URLs, or sibling symbols rather than inlining the URL in prose. Pin URLs to a specific commit/tag when citing upstream source so the link doesn't drift.
