...
2. **The revision proof, the fallback, and the cache reuse are covered by controlled acceptance** in the plugin's own suites: `reader-borrow.test.ts` (bytes equal the file's, same-size different bytes refused, a moved file re-proven, a replaced document borrowed again, a closed reader withdrawn, an oversized file refused, an open cancelled in flight closing its handle, and every wait the borrow owns bounded — a file stat that never settles, a handle that will not close, and a cancelled open that never answers each end the borrow), `renderer-borrow.test.ts` (a reader-backed crop takes no document load, publishes the bytes and cache identity the detached path publishes, falls back on an unproven, absent, closed, or pre-startup-and-stale document, cancels with its caller, keeps the queue's one-crop-at-a-time admission, and gives another PDF its turn after four borrowed crops), and `pdf-annotation-editor/borrow.test.ts` (the registry's accessor, closure, replacement, and the reader's own session standing while a document is lent).
