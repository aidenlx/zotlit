...
| `.github/workflows/release.yml` | push to `main` / `next` | parallel `obsidian` + `zotero` jobs: build, create the GitHub release, refresh the rolling Zotero update channel, attest provenance |
...
## Obsidian Language Packs: version-pinned release assets

Obsidian Language Packs are assets of the plugin's own release tag. Each
release workflow run generates the packs from the release commit and uploads
every locale except the base locale `en`, which ships bundled inside `main.js`.
An installed plugin composes its download URL from its own version, e.g.
`https://github.com/aidenlx/zotlit/releases/download/2.0.0/zh-CN.json`, so the
pack it downloads always matches its own message set (ADR 0018).

Upload, verification, and attestation all sit inside the tag-existence gate, so
a rerun on an already-released version changes no asset. After the upload the
workflow downloads each pack anonymously from its versioned URL and compares
the bytes; a mismatch fails the run and annotates the pack. The release is
already published at that point, so a failure here needs a manual decision —
delete the release and re-run, or publish a follow-up version. The steps use
only the built-in `GITHUB_TOKEN`; Language Pack downloads require no token.

> **A published version's assets are never rewritten.** The workflow uploads
> each pack exactly once, inside the gate, and nothing else touches it — so a
> translation fix ships with the next plugin release, not into a release
> already out.
...
3. On merge, `release.yml` runs. Each job's **tag-existence gate** makes
   versioned releases idempotent: every step, Language Pack upload included,
