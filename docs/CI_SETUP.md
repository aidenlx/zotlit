...
| `.github/workflows/release.yml` | push to `main` / `next` | parallel `obsidian` + `zotero` jobs: build, create the GitHub release, refresh rolling assets, attest provenance |
...
## Obsidian Language Packs: the rolling `language-packs` host

Obsidian Language Packs are served from a single permanent GitHub release
tagged `language-packs`. Every release workflow run generates and
clobber-uploads the JSON files, including `en.json` and `zh-CN.json`, then
downloads each fixed asset URL anonymously and verifies its bytes.

The workflow creates the rolling release when it is missing. It uses only the
built-in `GITHUB_TOKEN`; Language Pack downloads require no token.

> **Do not delete or rename the `language-packs` tag or release.** Installed
> copies use fixed URLs such as
...
3. On merge, `release.yml` runs. Each job's **tag-existence gate** makes
   versioned releases idempotent. The Obsidian job still regenerates and
   refreshes the rolling `language-packs` release so reruns publish the
   generated bytes. The Zotero job is a no-op when its `zt-<version>` tag
   already exists.
