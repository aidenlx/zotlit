...
- All held values use the same changed, settled, and invalidated event contract. A changed event means the value differs. A settled event also reports equal and failed reads, and carries the resulting Held Read or `null` while a failed first read stays pending.
