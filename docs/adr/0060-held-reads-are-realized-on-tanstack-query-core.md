...
- **A superseded read still cannot publish, by cancellation.** Query Core commits an in-flight result and clears the invalidated flag, so an invalidation that lands mid-read would be lost. An owner cancels the in-flight read with revert before it invalidates. Joined readers see a cancellation and ask again.
