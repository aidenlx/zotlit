...
4. Insert a citation through the normal workflow. Render the active original partial with the same selected Annotation used in its repair preview. Check that neither output contains a repair marker. The inactive previews must show the corresponding markers.
...
For stale-review coverage, start a separate `upgrader` trial. Prepare and review a copy, then append `ORIGINAL-CHANGED` to the original note source. Attempt acceptance. Check that activation stops and requires a new review against the changed original. Repeat with a relevant original frontmatter setting changed through a development probe; record and restore that setting after the trial.
