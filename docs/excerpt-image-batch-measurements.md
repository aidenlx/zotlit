...
Read against the pass that shares nothing, the batch retention **avoids 60 of 72 renders (83 %)** when the store cannot keep the bytes — the case ADR 0051 calls out, where the successful pixels exist only in memory. When the store does work, the batch retention does not change the render count: the store answers the repeats instead, which is why `repeated` and `perNote` both render 12. The retention's 60 hits there also spare the store a read: the job's probe takes the freshness stamp, consults the retained outcome, and reads `persistent.get` only on a miss, so each of those 60 requests was answered from memory without a store transaction. What the retention adds is the case the store cannot serve: it holds what a store whose write failed never kept, and its 60 hits are what takes `storeFailure` from 72 renders to 12.
