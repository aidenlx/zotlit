...
export function Tabs({
  ...
  /**
   * `useEffectEvent` returns a fresh closure on every render, so an effect that
   * lists it as a dependency re-runs on every render. Every closure dispatches
   * through the same internal ref, so pinning the first one keeps the identity
   * stable while still reaching the latest `onValueChange`. Without it the hash
   * effect below re-runs after each render and scrolls the page back to the
   * open tab — including the render that `updateAnchor` itself triggers.
   */
  const setValue = useRef(latestSetValue).current;
