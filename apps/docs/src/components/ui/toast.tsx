// shadcn Base UI Toast, using the site tokens and shared notification manager.
...
function Toast({ className, ...props }: ToastPrimitive.Root.Props) {
  return (
    <ToastPrimitive.Root
      data-slot="toast"
      className={cn(
        "group/toast pointer-events-auto absolute end-0 bottom-0 z-[calc(1000-var(--toast-index))] w-full origin-bottom rounded-2xl border border-fd-border bg-fd-popover text-fd-popover-foreground shadow-lg will-change-transform outline-none select-none focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-fd-primary motion-reduce:transition-none!",
...
function ToastDescription({
  ...
  return (
    <ToastPrimitive.Description
      ...
      className={cn("text-sm break-words text-fd-muted-foreground", className)}
      {...props}
    />
  );
}
...
function ToastClose({
  ...
  return (
    <ToastPrimitive.Close
      ...
      render={render}
      className={cn(
        "relative shrink-0 text-fd-muted-foreground hover:text-fd-foreground",
        className,
      )}
      {...props}
    >
