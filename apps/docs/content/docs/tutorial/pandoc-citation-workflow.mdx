...
<Steps>
  <Step>
    ...
    In Obsidian, run <Command inline>ZotLit: Open literature note quick switcher</Command>. Select each sample item to create its Literature Note.
...
For native error-code definitions, open **Pandoc CLI guide** under **Settings > ZotLit > Citations > Formatting**. The [Settings reference](/docs/reference/settings#native-pandoc) identifies the local guide and integration controls.
