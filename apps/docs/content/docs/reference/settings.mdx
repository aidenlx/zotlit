...
**Library scope:** The libraries ZotLit uses for item search, citation key resolution, and library-wide commands. **All libraries** follows your Zotero database, so a new group library joins on its own. **Selected libraries** stays exactly as you set it. An upgrade from an earlier ZotLit version starts at **Selected libraries** with **My Library**, whatever library you had selected before.
