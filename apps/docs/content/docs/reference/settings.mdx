...
| **Open Pandoc citations as links** | `citation.open-pandoc-links` | Toggle  | On      | **Pandoc citations** is on and the cursor is on a recognized Pandoc citation | Synced  |
...
**Open Pandoc citations as links:** Open literature notes from Pandoc citations. This setting does not change how citations are shown or what hovering them shows.

The navigation setting also controls the four open-under-cursor commands. **Hover action** owns hover on its own. Literature Note wikilinks retain Obsidian's navigation.
...
| **Source mode**  | `citation.hover-require-mod-source`       | Toggle                                         | On               | **Hover action** = Citation popover | Synced  |
| **Live Preview** | `citation.hover-require-mod-live-preview` | Toggle                                         | Off              | **Hover action** = Citation popover | Synced  |
| **Reading view** | `citation.hover-require-mod-reading`      | Toggle                                         | Off              | **Hover action** = Citation popover | Synced  |
...
**Source mode**, **Live Preview**, and **Reading view**: Show the popover only while holding Ctrl (Windows) or Command (macOS).

The three toggles appear only while **Hover action** is Citation popover. Each one gates the popover in its own editing mode, and the modifier is read when the pointer arrives.
