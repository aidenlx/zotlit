...
| **Show wikilink citations as citation text** | `citation.wikilink-display`   | Toggle  | On      |

**Citation key indexing** enables Citekey Indexing. It tracks literal Pandoc citation keys such as `@doe2024` across your vault. It is the master switch for literal citation key features in the references sidebar, editor, and reading view.

**Wikilink citations** enables Wikilink Citations in the references sidebar. It also allows fragment-less Literature Note wikilinks to use Citation Display Text.

**Show wikilink citations as citation text** changes a fragment-less Literature Note wikilink from its path to Citation Display Text. This applies in Live Preview and reading view. Both wikilink settings must be on. This setting is visible only while **Wikilink citations** is on.

A wikilink with a valid `#cite:` Citation Fragment displays as a citation regardless of both wikilink settings. Pandoc export also converts Literature Note wikilinks regardless of these settings.
...
**Citation key styling** styles citation keys such as `@doe2024` in the editor. It also enables Citekey Navigation and its command-palette commands. Live Preview and reading view show formatted citation text. See [How citation display and navigation work](/docs/concepts/how-citekey-links-work).

Hover preview is available when one citation key resolves to exactly one existing Literature Note. Configure its modifier gating under Obsidian's **Settings → Core plugins → Page preview**, using the source **ZotLit citation key**.
