...
| `zt.annotations`  | `array`          | All annotations across all attachments (see [Annotation template](#annotation-template)). Entries carry every property listed there except `citation`, which only a single-annotation render resolves |
...
| `zt.indexedKey`       | `string`           | `zt.key` for the personal library, `KEYgGROUPID` for a group library                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `zt.libraryID`        | `number`           | Library ID                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `zt.type`             | `string`           | `"highlight"`, `"note"`, `"image"`, `"ink"`, `"underline"`, or `"text"`; `"unknown"` for a type Zotero added after this release                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| `zt.parentItem`       | `object \| null`   | The parent literature item, resolved like a full item: its own fields plus `backlink`, `weblink`, `authors`, and `authorsShort`. `null` for annotations on standalone attachments. `notePath`/`noteLink` read as unresolved on standalone annotation renders (annotation view drag/insert, note import) — resolving the real note path there would require vault I/O the synchronous drag path avoids. `parentItem.collections` is populated when the annotation renders as part of a note; in standalone renders collections are absent; read them via `zt.collections` on the note context instead |
| `zt.parentAttachment` | `object`           | The parent attachment (see [Attachments](#attachments))                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `zt.citation`         | `string \| null`   | Page-pinned citation rendered through your cite template with the annotation's `pageLabel` as locator. `null` when there is no parent item or no citation key. Available on this root alone, so `zt.annotations` entries in a note or content template have none                                                                                                                                                                                                                                                                                                                                     |
...
| `indexedKey`  | `string`         | `key` for the personal library, `KEYgGROUPID` for a group library                                        |
...
| `backlink`    | `string`         | Zotero deep link to open this attachment in the reader (`zotero://open/...`)                             |
