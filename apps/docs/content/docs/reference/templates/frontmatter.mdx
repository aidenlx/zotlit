...
| `citekey`    | `zt.citationKey` | Compatibility frontmatter key for the citation key. Written only when the item has one; deleted otherwise |
...
| `citekey`         | Compatibility frontmatter key; system-managed (literature note identity) |
...
1. **System fields** (`zotero-key`, compatibility frontmatter key `citekey`) are refreshed.
2. **User-configured fields** are re-evaluated and applied using their merge strategy.
