...
The name resolves to a registered template. During Profile rendering, `"annotation"` selects the Profile's Annotation Section. Outside Profile rendering, it looks up `zotlit-annotation.liquid.md`. Other names keep their existing lookup; each is a name, not a file path.
...
When both a `.liquid.md` and a `.eta.md` file exist for the same template name, the Liquid file takes precedence. The Eta file is ignored regardless of whether <UiLabel name={m.settings_template_js_enable_name()} /> are enabled.

<SettingsPath page={m.settings_page_templates()} /> reports a warning on the
shadowed file:
