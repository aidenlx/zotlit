...
Renders the named `annotation` template with one annotation available as `zt` inside that template.
...
The shortcut uses the same named-template lookup as [`{% render %}`](#includes-with-render). A Profile's Annotation Block remains a separate rendering target. Native Liquid syntax remains available for custom variable names and other template composition.
