...
Pass `zt.citations` to the Pandoc Citation formatter:
...
The formatter uses `prefix`, `suffix`, `suppressAuthor`, `locator`, and `labelShort`. It omits entries whose `item.citationKey` is null.
