...
`zt-citation-key` and its state classes identify active literal Pandoc sources. **{m.settings_citation_pandoc_citations_name()}** controls their membership. **{m.settings_citation_show_formatted_name()}** and **{m.settings_citation_open_as_links_name()}** control the two features that can activate their hooks.

`zt-literature-note-link` identifies active Wikilink Citation sources. **{m.settings_citation_wikilink_citations_name()}** controls their membership. ZotLit applies the hook when **{m.settings_citation_show_formatted_name()}** supplies their rendering. Obsidian supplies wikilink navigation while **{m.settings_citation_open_as_links_name()}** is on; when it is off, a plain click on a rendered wikilink citation puts the cursor in the link's source text in Live Preview and does nothing in reading view, and Mod+click still opens the note.
...
The class appears once more in the {m.settings_citation_hover_action_popover()} of such a Citation, on the element that labels each cited item's button row with that item's serial. The same `⚠` stands there for a work the bibliography rendered no entry for.
...
Ambiguous citation keys keep the resolved link color and take the `--link-unresolved-decoration-*` variables, so the key reads as reachable — the {m.settings_citation_hover_action_popover()} names its candidates — while stating that it matches more than one item.

ZotLit adds no style to `zt-entry-serial` either. The serials render as superscript text in a Citation, as a plain label in the {m.settings_citation_hover_action_popover()}, and take their appearance from the theme.
