...
**Citation key styling** in **Settings → ZotLit → Citations** makes citation keys clickable. It is on by default, and takes effect while **Citation key indexing** is also on.

Type or paste a Pandoc citation like `@key` or `[@key]` in Live Preview or Source mode. Click its citation key to open or create the corresponding Literature Note.
...
For the recognized syntax and what reading view shows, see [How citation key styling works](/docs/concepts/how-citekey-links-work).
