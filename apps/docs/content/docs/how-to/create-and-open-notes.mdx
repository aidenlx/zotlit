...
The **Citation key property** setting controls direct lookup and defaults to `citekey`. Exactly one matching Literature Note supports direct opening. Files without `zotero-key` do not count as Literature Note matches.
