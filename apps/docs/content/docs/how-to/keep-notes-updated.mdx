...
To update every note at once, run <Command inline>ZotLit: Create or update all literature notes</Command> from the Obsidian command palette. This skips the Zotero selection step and covers every library in your [library scope](/docs/reference/settings#library-scope).
...
To fully refresh a v1 note's body, use <Command inline>ZotLit: Overwrite literature note</Command>. This replaces the entire body with the current note template.
