...
<Steps>
  ...
  <Step>
    ...
    Select a CSL style installed in Zotero. Enter the complete document's language, then select **{m.citation_presentation_confirm()}**.
...
Open **{m.settings_citation_pandoc_guide_name()}** under **Settings > ZotLit > {m.settings_page_citations()} > {m.settings_citation_references_heading()}** for the full error reference.
