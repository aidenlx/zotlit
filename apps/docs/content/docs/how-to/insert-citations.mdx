...
With the opt-in **At trigger** enabled, a bare `@` at a word boundary (start of line, after a space, or after an opening bracket or quote) also opens the suggester.
...
<Steps>
  <Step>
    ...
    With **At trigger** enabled, typing `@` at a word boundary does the same thing. A mid-word `@` (e.g. in an email address) is ignored.
...
- **At trigger** (default: off): also opens the suggester on a bare `@` at a word boundary. Visible only while **Citation suggester** is on. Find it at the top of **Settings > ZotLit**.
- **Show citation key in suggestions** (default: off): shows each item's citation key beside its title in the suggester list. Find it at the top of **Settings > ZotLit**.
...
Only the bracketed primary form becomes a clickable [citation key link](/docs/concepts/how-citekey-links-work) in the editor. The default secondary form is bare, so it stays plain text.
