...
<Steps>
  ...
  <Step>
    ...
    ZotLit manages reserved frontmatter keys that you cannot target from a user field. For literature notes: `zotero-key` and `citekey`. For imported notes: `zotero-note-key` and `zotero-lastmod`. See [Frontmatter reference](/docs/reference/templates/frontmatter) for details.
