...
Two settings under **Settings > ZotLit > {m.settings_page_citations()}** decide which sources join the set:

| Setting                                             | Default | Included source                                           |
| --------------------------------------------------- | ------- | --------------------------------------------------------- |
| **{m.settings_citation_pandoc_citations_name()}**   | On      | Literal Pandoc syntax such as `@doe2024` and `[@doe2024]` |
| **{m.settings_citation_wikilink_citations_name()}** | Off     | Eligible wikilinks to Literature Notes                    |
...
Aliases, embeds, unresolved targets, heading fragments, and block fragments stay outside the set. Malformed Citation Fragments also stay outside it. The References Sidebar reports a malformed fragment as a Reference Error while **{m.settings_citation_wikilink_citations_name()}** is on.
...
ZotLit resolves Pandoc citation keys against Zotero's native citation key field, across every library in your **{m.settings_library_scope_name()}**. It does not use the `citekey` frontmatter field for resolution.
...
**{m.settings_citation_show_formatted_name()}** is on by default. It applies the selected **{m.settings_citation_references_style_name()}** in Live Preview and reading view. Source mode always shows Markdown.
...
Turning **{m.settings_citation_show_formatted_name()}** off leaves every source in Obsidian's native presentation. It does not change set membership, Citekey Navigation, insertion, or export.
...
**{m.settings_citation_open_as_links_name()}** controls navigation for both literal Pandoc citations and Literature Note wikilinks shown as citations. It works independently of formatted display and hover behavior.
