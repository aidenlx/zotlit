...
Live updates are a push channel. The companion sends HTTP notifications to Obsidian when items change, annotations are added, or you navigate in the Zotero PDF reader. These notifications power the annotation view's reader-following mode and tell ZotLit to refresh its database connection.

Live updates do not write recent changes into the main Zotero database file. The companion's database checkpoint handles that work independently.
