...
Zotero 10 changed how it stores recent edits, so changes made while Zotero was open could stay invisible to ZotLit. The Companion now keeps the database current for ZotLit automatically. When you switch from Zotero to Obsidian, your changes are already there, whether or not live updates are enabled. A new <UiLabel name={m["zotero.database_status.tooltiptext"]()} /> panel on the Zotero sidebar confirms that recent changes have reached the database and lets you trigger a manual update if something went wrong. See [Fix Zotero changes that do not appear](/docs/how-to/fix-stale-data) for details.
