...
You can now create or update literature notes and import Zotero notes across an entire library or a single collection (including subcollections), instead of selecting items one at a time. Right-click a library or collection in Zotero's sidebar to find <UiLabel name={m["zotero.menu_collection_update_all.label"]()} /> and <UiLabel name={m["zotero.menu_collection_import_all_notes.label"]()} /> in the <UiLabel name={m["zotero.menu_submenu.label"]()} /> submenu. The same actions are available as Obsidian commands and protocol links (`update-all`, `import-all-notes`).
...
- The "Receive updates from Zotero" toggle now shows a hint linking to the install guide for ZotLit Companion, the Zotero add-on, when the Companion is not yet set up.
