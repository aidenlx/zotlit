...
- `pnpm exec turbo run dev --filter=@zotlit/docs` — build workspace dependencies, then Vite.
...
- `pnpm --filter @zotlit/docs deploy:beta` — the same for Pre-release Docs. `CLOUDFLARE_ENV` picks the environment at **build** time, so the beta line needs its own build; without it the bundle carries production's variables whatever `wrangler deploy --env` says.
- `pnpm --filter @zotlit/docs cf-typegen` — regenerate the committed `worker-configuration.d.ts` after any `wrangler.jsonc` binding, variable, secret, or compatibility change. The typecheck fails when it is stale.
...
- **Template Workbench:** [`src/routes/workbench.tsx`](src/routes/workbench.tsx) is a prerendered static shell, so `/workbench` stays off `run_worker_first` and costs no Worker invocation. The editor bundle, the `@zotlit/workbench/ui` tree it themes, which module owns which surface, and the dev-server wiring are in [`src/lib/workbench/README.md`](src/lib/workbench/README.md) — read it before editing that directory.
...
- **Release availability:** [`src/lib/docs-availability.ts`](src/lib/docs-availability.ts) reads a page's `introduced` / `updated` against the Docs Release Line in [`zotlit-release.json`](zotlit-release.json) and stays server-side, so `semver` never reaches the browser. The `/docs` route shell decorates the page tree before it crosses the `createServerFn` boundary and [`src/layouts/docs/slots/sidebar.tsx`](src/layouts/docs/slots/sidebar.tsx) renders the resulting `NEW` / `UPDATED` pill; [`src/components/docs-page.tsx`](src/components/docs-page.tsx) hands the page's availability to [`src/components/docs-availability.tsx`](src/components/docs-availability.tsx) for the `AVAILABLE SINCE` row; [`src/lib/markdown-editions.tsx`](src/lib/markdown-editions.tsx) leads each docs edition and `llms-full.txt` with the `_Available since ZotLit …._` preamble.
...
- **Fonts:** Fontsource serves all four faces from this package's own assets, registered and role-assigned in `src/styles.css`. Its metric-adjusted local fallback faces preserve the shift-free swaps `next/font` gave before the TanStack Start migration, so only Gelasio is preloaded (in [`src/routes/__root.tsx`](src/routes/__root.tsx)); Inter and IBM Plex Mono ride their fallbacks, and the Archivo wordmark falls under `assetsInlineLimit` and is inlined.
...
- Each suite sits beside what it pins, and its names say which behaviour. Run them through turbo so dependencies build first. The Workbench splits by surface: `web-connection` (the Local Bridge, hydration, Save), `web-editor` (problem navigation, the sample paper, the editing flow, the annotation box), `web-draft` (the restore prompt, handoff, import and download), `web-frame` (the narrow layout), `web-preview` (render scheduling), and `workbench.test.tsx` (StrictMode replay, and what no surface owns alone). The editor panes live in the shared tree, so their suites run in `@zotlit/workbench` under React and Preact. The Fixture Local Bridge test also connects through the real loopback server after a browser CORS preflight.

## Deployment

- Two lines, both in [`wrangler.jsonc`](wrangler.jsonc): production from `main`, and the `beta` environment (Pre-release Docs, `DOCS_LINE=beta`) from `next`. [`.github/workflows/docs-deploy.yml`](../../.github/workflows/docs-deploy.yml) picks the environment by branch for both the build and the deploy, and a gate job asks turbo whether the push touched `@zotlit/docs` or its dependency closure. It needs the `CLOUDFLARE_API_TOKEN` and `CLOUDFLARE_ACCOUNT_ID` repository secrets.
- Two values come from outside the repo: the `GITHUB_TOKEN` Worker secret (`wrangler secret put GITHUB_TOKEN`, per environment) and `VITE_CF_BEACON_TOKEN`, the public Cloudflare Web Analytics site token read at build time. Each line has its own Web Analytics site, so the workflow picks the beacon token by branch. Without the beacon token the site serves no analytics script.
- Five settings live in the Cloudflare dashboard rather than in `wrangler.jsonc`, and survive a deploy: each Worker route is set to **fail open**, so a Worker outage falls through instead of erroring; **AI Crawl Control** blocks the abusive crawler operators; one **WAF rate-limit rule** covers the HTML page paths; **Web Analytics** is enabled for the site token the build reads; and **Image Transformations** is enabled for the `aidenlx.site` zone. Giscus' allowed origins (`giscus.json` at the repo root) list both lines' domains, so blog comments load on Pre-release Docs as well.
...
**Images:** Render JSX images through [`DocsImage`](src/components/docs-image.tsx) or `@unpic/react`. Put screenshots under `public/img/<collection>/` as `.webp`, reference them with root-relative `/img/...` paths, and convert source images with `cwebp`. Keep the pipeline aligned across [`source.config.ts`](source.config.ts) for MDX metadata and [the deploy workflow](../../.github/workflows/docs-deploy.yml) for Git LFS checkout.
...
Leave `introduced` and `updated` unset when you author a page. Both are optional; an unset page carries no badge and no "Available since" line until it ships.

`pnpm docs:availability <stable-version>` is the sole writer of those fields — run it with `--help` for its flags and contract, and `--check` to scan without writing. Section Index pages (`index.mdx`), generated pages, and `_`-prefixed partials stay outside its scan. On a stable Obsidian release `release.ts` offers it as a handoff and itself writes only the Docs Release Line in `zotlit-release.json`, which the site derives `NEW` and `UPDATED` from. See [ADR 0002](docs/adr/0002-release-availability-is-git-diff-assisted-not-hand-authored.md).
