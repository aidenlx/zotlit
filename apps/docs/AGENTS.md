...
- **Template Workbench:** [`src/routes/workbench.tsx`](src/routes/workbench.tsx) is a prerendered static shell; the editor bundle in [`src/lib/workbench/`](src/lib/workbench) — the document core, CodeMirror, and the render Worker — loads after mount, so `/workbench` stays off the `run_worker_first` list and costs no Worker invocation. Rendering runs in [`render-worker.ts`](src/lib/workbench/render-worker.ts), a module Worker (`worker.format` in [`vite.config.ts`](vite.config.ts)) that polyfills `Temporal` when the runtime has none. Editing rules, slice editors, and render scheduling live in `@zotlit/workbench`; this app owns the layout and the Worker transport. The result column is a reading view: [`reading-view.tsx`](src/lib/workbench/reading-view.tsx) is the one module that reads the parser packages' node types — a unified pipeline (remark-gfm, the pinned quartz-community Obsidian remark and rehype packages, rehype-callouts) into an inert hast tree, and the hast-to-JSX map that paints it. Wikilinks, tags, and links carry no target; an embed becomes an unavailable placeholder chosen by its file type, because a snapshot carries no vault files. The **Markdown** toggle beside the heading shows the generated Markdown byte for byte, which stays the reference whenever the reading view is approximate. Every parser package is pinned to an exact version so a swap stays inside that file. The field column is the Template Data Explorer in a value-first form: [`fields.ts`](src/lib/workbench/fields.ts) owns the root an editor position writes against — the note body, the Annotation Section, or the manifest's `filename` value — and the common rows each root leads with; [`field-list.tsx`](src/lib/workbench/field-list.tsx) paints them and, behind **Everything else from Zotero**, the whole Explorer tree over the same snapshot. Rows and tree share one search box, one snippet, and one insertion: **Put in note** patches the master buffer at the selection the editor last reported and hands focus back, and typing `{{` opens the same list over the two braces it inserts across. The tree, its filter, and the snippet kinds come from `@zotlit/workbench/explorer`, the modules the Obsidian sidebar runs on; [`fields.test.ts`](src/lib/workbench/fields.test.ts) pins the rows, their values, and the root each offset resolves to.
