...
export function ChipInput({
  ...
  return (
    <div
      className={cn(
        "zt:col-span-4 zt:row-start-2 zt:flex zt:min-h-(--input-height) zt:w-full zt:min-w-0 zt:flex-1 zt:basis-64 zt:flex-wrap zt:items-center zt:gap-1 zt:border-s-0 zt:border-t zt:border-border zt:px-1.5 zt:py-1 zt:@xl:col-span-1 zt:@xl:col-start-3 zt:@xl:row-start-1 zt:@xl:w-auto zt:@xl:border-s zt:@xl:border-t-0",
        hint && "zt:items-start",
      )}
      data-chip-input=""
      onMouseDown={(event) => {
    ...
    >
      {values.map((value, index) => (
        <div
          key={index}
          className={cn(
            "zt:flex zt:max-w-full zt:flex-col",
            index === values.length - 1 &&
              "zt:max-w-[calc(100%_-_2ch_-_0.25rem)]",
          )}
        >
          <span className="zt:flex zt:max-w-full zt:items-center zt:gap-0.5 zt:self-start zt:rounded-sm zt:bg-muted zt:py-0.5 zt:ps-1.5 zt:text-sm zt:leading-tight">
          ...
          {hint?.(value) && (
            <span className="zt:px-1.5 zt:pt-0.5 zt:text-xs zt:leading-tight zt:text-muted-foreground">
              {hint(value)}
            </span>
          )}
        </div>
