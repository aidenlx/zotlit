...
const logger = getLogger(["setting-tab", "compat", "database"]);
...
/**
 * "Zotero database" section: connection, status, and read mode, with an
 * "Advanced" sub-heading for the data-dir and source-id rows. Obsidian's
 * declarative tab sets each row's name/desc from the definition before calling
 * `render(setting)`; here we set the name on the `Setting`, then let the ported
 * render helper set the desc + control and return a cleanup.
 */
export function databaseSection(
  ...
  const group = sectionGroup(
    containerEl,
    m.settings_page_database(),
    m.settings_page_database_desc(),
  );

  group.addSetting((setting) =>
    ctx.defer(
      renderProfileDirRow(
        setting.setName(m.settings_db_profile_dir_name()),
        ctx,
      ),
    ),
  );
  group.addSetting((setting) =>
    ctx.defer(
      renderDatabaseFileRow(setting.setName(m.settings_db_file_name()), ctx),
    ),
  );
  group.addSetting((setting) =>
    ctx.defer(
      renderReadModeRow(setting.setName(m.settings_db_read_mode_name()), ctx),
    ),
  );
  group.addSetting((setting) =>
    setting
      ...
      .addToggle((toggle) =>
        toggle
          .setValue(ctx.settings.current?.["zotero.auto-refresh"] ?? true)
          .onChange((value) =>
            ctx.settings.update({ "zotero.auto-refresh": value }),
          ),
      ),
  );

  const advanced = sectionGroup(group.listEl, m.settings_db_advanced());
  advanced.addSetting((setting) =>
    ctx.defer(
      renderDataDirRow(setting.setName(m.settings_db_data_dir_name()), ctx),
    ),
  );
  advanced.addSetting((setting) =>
    ctx.defer(
      renderSourceIdRow(setting.setName(m.settings_db_source_id_name()), ctx),
    ),
  );
}
...
function renderReadModeRow(setting: Setting, ctx: CompatContext): () => void {
  const stack = new DisposableStack();
 ...
 */
function renderProfileDirRow(setting: Setting, ctx: CompatContext): () => void {
  const stack = new DisposableStack();
 ...
 */
function renderDataDirRow(setting: Setting, ctx: CompatContext): () => void {
  const stack = new DisposableStack();
  const pref = ctx.zoteroPref;
 ...
 */
function renderSourceIdRow(setting: Setting, ctx: CompatContext): () => void {
  const stack = new DisposableStack();
  const pref = ctx.zoteroPref;
