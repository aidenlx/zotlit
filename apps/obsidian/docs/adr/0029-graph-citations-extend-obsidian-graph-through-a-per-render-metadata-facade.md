...
- The render wrapper depends on the engine reading its app reference once at the top of render; the edge tint proxy depends on the link sprite lifecycle. The Work Label wrapper also depends on the node text sprite lifecycle: `initGraphics` builds text lazily, `clearGraphics` destroys it, and render assigns its style on font changes. These are the re-verification points.
