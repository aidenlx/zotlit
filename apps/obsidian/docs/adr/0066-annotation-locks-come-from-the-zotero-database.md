...
- Unknown facts mean not locked — the database is unreadable, the Annotation is not yet in the copy ZotLit reads, or no account `userID` exists. Zotero itself counts a missing creator or current user as the author. A new External Annotation stays editable until the copy is read again.
