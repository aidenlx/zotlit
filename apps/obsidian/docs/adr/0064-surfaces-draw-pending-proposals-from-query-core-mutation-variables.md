...
- `MutationState` reads its `pending` from the same mutation cache: an Annotation is pending from the moment a write is asked for until the last write queued on it settles, and names the write in flight. The repository keeps only the failure or conflict a settled write leaves standing, and announces `mutation-changed` in the same task as a write starts and settles. A queue of writes on one Annotation therefore never shows idle between them.
