...
- A native `MenuItem` carries no tooltip, so a blocked entry states its reason in a `setIsLabel(true)` line beside it, which is what `pane-menu.ts` and `actions.tsx` already do. See [policies/tooltips.md](../../policies/tooltips.md).
