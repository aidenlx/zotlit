...
- **Prerendering:** [`src/lib/prerender-pages.ts`](src/lib/prerender-pages.ts) lists the Markdown surface for `tanstackStart({ pages })`, so the asset layer answers those routes without a Worker invocation. Vite loads its config outside the app's module graph, so that list scans the content directory instead of reading the collections. Automatic discovery stays off: the HTML routes still render on the Worker.
- **Pending slices:** the SEO endpoints, request-time GitHub data, and the "Manuscript & Machine" styling land in later tickets of issue #846. The docs-availability preamble (`_Available since ZotLit …_`) arrives with the availability port.

## Verification

- [`http.test.ts`](src/http.test.ts) is the primary seam: it serves `dist/` through workerd and asserts what a browser sees — page status, redirect targets, Markdown editions, search results, asset headers. It walks the page list from the loaders, so a page the prerender scan misses fails there. Run it through turbo so the build runs first.
